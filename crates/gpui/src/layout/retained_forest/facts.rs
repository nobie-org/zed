...
//!
//! These values are temporary GPUI layout intent: what the current render pass
...
#[derive(Clone, Debug, PartialEq)]
pub(super) enum LayoutIntentKind {
    Unmeasured { children: Vec<LayoutId> },
    Measured(MeasuredLayoutFacts),
}

/// Whether a layout intent's subtree can produce GPUI paint artifacts.
///
/// This is a GPUI fact, not solver state. `display: none` subtrees still exist as
/// layout intents, but GPUI skips prepaint/paint for their descendants, so the
